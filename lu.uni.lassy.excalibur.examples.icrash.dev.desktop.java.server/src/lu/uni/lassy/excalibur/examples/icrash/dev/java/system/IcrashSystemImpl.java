...
public class IcrashSystemImpl extends UnicastRemoteObject implements
	...
	public PtBoolean oeCreateSystemAndEnvironment(PtInteger aQtyComCompanies)
		...
		try {
			...
			*/
			...
			aKeyPair.getKeys();
			aKeyPair.getPrivateKey().saveToFile(new PtString(adminName));
			ctAdmin.init(aLogin, aPwd, aKeyPair.getPublicKey());
			/*
	...
	public PtBoolean oeAddCoordinator(DtCoordinatorID aDtCoordinatorID,
			DtLogin aDtLogin, DtPassword aDtPassword, DtPublicKey aDtPublicKey) throws RemoteException {
		try {
			//PreP1
			isSystemStarted();
			//PreP2
